 ...
 * supabase/migrations/20260730000200_floor_core_tables.sql. The logic is
 * duplicated on purpose: a scan has to resolve against the local cache with zero
 * network, so normalization cannot require a round trip. The database keeps its
 * own copy so nothing malformed can be written even by a caller that skips this.
 ...
 *
 * Returns null for anything unreadable. Null is a real answer, not a failure: an
 * unparseable scan still gets logged with hit = false, which is how a bad lens or
 * a run of damaged labels becomes visible.
 */
export function normalizeValpn(raw) {
  if (!raw) return null;
...
export function isCanonicalValpn(value) {
 ...
 * Cosmetic only — never store or compare this form.
 */
export function formatValpnForDisplay(valpn) {
  const digits = String(valpn || '').replace(/^VALPN-/, '');
  return `VALPN-${digits.replace(/(\d{4})(?=\d)/g, '$1 ')}`;
}
